...
class LeCartWoocommerce(LeCartWordpress):
	...
	def get_query_display_import_source(self, update = False):
		...
		queries = {
			...
			'customers': {
				'type': 'select',
				'query': "SELECT COUNT(1) AS count FROM " + prefix + "users u LEFT JOIN " + prefix + "usermeta um ON u.ID = um.user_id WHERE um.meta_key = '_DBPRF_capabilities' AND um.meta_value LIKE '%customer%' AND u.ID " + compare_condition + to_str(
	...
	def after_category_import(self, category_id, convert, category, categories_ext):
		...
		if self.is_polylang():
			...
			taxonomy_lang_id = self.get_map_field_by_src('category_lang', category_id)
	...
	def product_import(self, convert, product, products_ext):
		product_data = {
			...
			'to_ping': '',
			'pinged': '',
			'post_modified': convert['updated_at'] if convert['updated_at'] and '0000-00-00' not in convert['created_at'] else get_current_time(),
			'post_modified_gmt': convert['updated_at'] if convert['updated_at'] and '0000-00-00' not in convert['created_at'] else get_current_time(),
			'post_content_filtered': '',
			'post_parent': 0,
			'guid': '',
			'menu_order': get_value_by_key_in_dict(convert, 'menu_order', 0),
			'post_type': "product",
			'post_mime_type': '',
			'comment_count': 0,
		}
	...
	def get_customers_main_export(self):
		...
		query = {
			'type': 'select',
			'query': "SELECT * FROM " + prefix + "users u LEFT JOIN " + prefix + "usermeta um ON u.ID = um.user_id WHERE um.meta_key = '_DBPRF_capabilities' AND um.meta_value LIKE '%customer%' AND ID > " + to_str(id_src) + " ORDER BY ID ASC LIMIT " + to_str(limit)
		}
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		customer_data['capabilities'] = get_row_value_from_list_by_field(customer_meta, 'meta_key', 'wp_capabilities', 'meta_value')
		# billing
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		customer_data = {
			...
			'user_pass': convert.get('password'),
	...
	def after_customer_import(self, customer_id, convert, customer, customers_ext):
		...
		for meta_key, meta_value in user_meta_import.items():
			...
			queries_user.append(meta_query)
		if queries_user:
			self.import_multiple_data_connector(queries_user, 'customer')
